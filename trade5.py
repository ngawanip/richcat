...
def get_klines(pair, interval="1d", limit=30):
    """
    Silently handles the fact that Klines are unsupported on the Roostoo Mock API.
    Returns an empty list to gracefully trigger the bot's fallback strategies.
    """
    try:
        payload = {"pair": pair, "interval": interval, "limit": limit}
        r = requests.get(BASE_URL + "/v3/klines", params=payload, timeout=3)
...
def execute_strategy():
    ...
    for pair in pairs:
        ...
        if hist:  # Will only run if Klines actually exist
    ...
    if not lowest_risk_pair:
        print("  -> Primary Strategy Skipped (Historical data unavailable on this API).")
        print("  -> Activating Fallback: Searching for the cheapest asset...")
